      ...
      {messages.map((msg) => {
        ...
        return (
          <div
            key={msg.id}
            className={cn(
              "flex items-start gap-2 sm:gap-2.5 max-w-[90%] sm:max-w-[80%]",
              isLead ? "mr-auto" : "ml-auto flex-row-reverse"
            )}
          >
            {/* Sender Avatar */}
            <Avatar className="size-6 sm:size-7 shrink-0 text-xs mt-0.5">
              <AvatarFallback
                className={cn(
            ...
            <div className="space-y-1 min-w-0">
              <div
                className={cn(
                  "flex items-center gap-1.5 text-[10px] text-muted-foreground truncate",
                  !isLead && "justify-end"
                )}
              >
              ...
              <div
                className={cn(
                  "rounded-2xl px-3 sm:px-3.5 py-2 text-xs leading-relaxed shadow-xs break-words overflow-hidden",
